...
const circuitWord = (d) =>
  (d?.judicialCircuit || d?.circuit || "THIRTEENTH").toString().toUpperCase() === "13TH"
    ? "THIRTEENTH"
    : (d?.judicialCircuit || d?.circuit || "THIRTEENTH").toString().toUpperCase();
  ...
  "name-change-hillsborough": {
    ...
    fields: {
      ...
      "12-928-cover": {
        ...
        "indicate county": (d) => (d.county || "HILLSBOROUGH").toString().toUpperCase(),
        "case number": () => "",                                 // clerk fills
        "judges name": () => "",                                 // clerk fills

        "petitioner name": (d) => currentFull(d),
        "respondent name": () => "",                             // N/A for adult name change packet

        // Signature line MUST be blank; printed name is separate field below
        "attorney or party signature": () => "",
        "florida bar number": () => "",

        // This is the "(Type or print name)" line
        "indicate your name here": (d) => currentFull(d),

        // Case type checkbox
        "name change": () => true,

        // Leave optional / unrelated boxes unchecked
        ...
        // Related cases yes/no — we’re holding off, so leave both blank
        "check no": () => false,
        "check yes": () => false,

        // Nonlawyer section should be blank by default
        "nonlawyer name": () => "",
      ...
      "12-982a-petition": {
        // Caption
        "Judicial Circuit": (d) => circuitWord(d),               // THIRTEENTH
      ...
      /* stubs for other PDFs (we’ll fill next) */
      "12-982b-judgment": {},
      "12-900h-related":  {},
      "12-915-address":   {},
      "dh-427-report":    {}
